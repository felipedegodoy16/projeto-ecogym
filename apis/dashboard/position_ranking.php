<?php 
  ...
  $sql = "SELECT ROW_NUMBER() OVER (ORDER BY SUM(m.CALORIA_GASTA) DESC) AS POSICAO, SUM(m.CALORIA_GASTA) AS CALORIA, u.NOME, u.CPF, u.ID_USUARIO FROM movimento m INNER JOIN usuarios u ON m.FK_USUARIO_ID = u.ID_USUARIO GROUP BY FK_USUARIO_ID ORDER BY SUM(m.CALORIA_GASTA) DESC;";
